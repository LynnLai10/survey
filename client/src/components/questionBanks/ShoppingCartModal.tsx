      ...
      <div className="bg-white w-full h-full flex flex-col">
        ...
        <div className="flex items-center justify-between p-6 border-b border-gray-200">
          <div className="flex items-center space-x-3">
            ...
            <h2 className="text-2xl font-bold text-gray-900">
              {t('questionBanks.marketplace.shoppingCart', 'Shopping Cart')} ({totalItems})
            </h2>
          </div>
          <button
            onClick={handleClose}
          ...
          {items.length === 0 ? (
            <div className="text-center py-12">
              ...
              <p className="text-gray-500">
                {t('questionBanks.marketplace.cartEmptyDescription', 'Add some question banks to get started')}
              </p>
            </div>
          ) : (
                  ...
                  <div className="flex-1">
                    ...
                    <div className="flex items-center space-x-2 mt-2">
                      <span className={`px-2 py-1 text-xs font-semibold rounded-full ${
                        ...
                        {item.type.toLowerCase() === 'free' ? 'Free' : `$${item.price?.toFixed(2)}`}
                      </span>
                      {item.questionCount && (
                        <span className="text-xs text-gray-500">{item.questionCount} questions</span>
                      )}
                    </div>
                  </div>
                  <button
        ...
        {items.length > 0 && (
          <div className="border-t border-gray-200 p-6">
            <div className="flex items-center justify-between mb-4">
              ...
              <div className="text-right">
                <p className="text-sm text-gray-500">
                  {t('questionBanks.marketplace.totalItems', 'Total Items')}: {totalItems}
                </p>
              </div>
            </div>
            <button
            ...
            >
              {t('questionBanks.marketplace.proceedToCheckout', 'Proceed to Checkout')}
            </button>
          </div>
        )}
      </div>
