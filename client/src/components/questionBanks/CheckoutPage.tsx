  ...
  const handleConfirmPurchase = async () => {
    ...
    try {
      ...
      if (failedItems.length > 0) {
        const errorMessage = failedItems.map(item => `${item.item}: ${item.error}`).join('\n');
        alert(`Some items failed to process:\n${errorMessage}`);
        
        // If some succeeded, show partial success message
        if (successItems.length > 0) {
          alert(`Successfully processed: ${successItems.map(item => item.item).join(', ')}`);
        }
      } else {
      ...
      <div className="max-w-4xl mx-auto px-4">
        ...
        <div className="bg-white rounded-lg shadow-sm p-6 mb-6">
          ...
          <p className="text-gray-600 mt-2">
            {t('questionBanks.marketplace.checkoutDescription', 'Review your order and complete your purchase')}
          </p>
        </div>
                    ...
                    {freeItems.map((item) => (
                      <div key={item._id} className="flex items-center justify-between p-3 bg-green-50 rounded-lg">
                        <div>
                          ...
                          <p className="text-sm text-gray-600">{item.questionCount} questions</p>
                        </div>
              ...
              <div className="border-t border-gray-200 pt-4">
                ...
                {freeItems.length > 0 && (
                  <p className="text-sm text-green-600 mt-1">
                    {freeItems.length} free item{freeItems.length !== 1 ? 's' : ''} included
                  </p>
                )}
              </div>
            </div>
            ...
            <div className="bg-white rounded-lg shadow-sm p-6 sticky top-6">
              ...
              <div className="space-y-4">
                <div className="text-sm text-gray-600">
                  <p>• {totalItems} item{totalItems !== 1 ? 's' : ''} in your order</p>
                  {freeItems.length > 0 && (
                    <p>• {freeItems.length} free item{freeItems.length !== 1 ? 's' : ''} will be added immediately</p>
                  )}
                  {paidItems.length > 0 && (
                    <p>• {paidItems.length} paid item{paidItems.length !== 1 ? 's' : ''} will be processed</p>
                  )}
                </div>
                ...
                >
                  {loading
                    ...
                    : totalAmount === 0
                    ? t('questionBanks.marketplace.claimFree', 'Claim Free Items')
                    : t('questionBanks.marketplace.completePurchase', 'Complete Purchase')
                  }
                </button>
                ...
                >
                  {t('questionBanks.marketplace.continueShopping', 'Continue Shopping')}
                </button>
              </div>
            </div>
          </div>
