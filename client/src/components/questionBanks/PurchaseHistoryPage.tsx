			...
			<div className='max-w-6xl mx-auto px-4'>
				...
				<div className='bg-white rounded-lg shadow-sm p-6 mb-6'>
					...
					<p className='text-gray-600 mt-2'>
						{t(
							'questionBanks.marketplace.purchaseHistoryDescription',
							'View your question bank purchases and access history'
						)}
					</p>
				</div>
				...
				<div className='bg-white rounded-lg shadow-sm mb-6'>
					<div className='border-b border-gray-200'>
						<nav className='flex space-x-8 px-6'>
							...
							>
								{t('questionBanks.marketplace.orderHistory', 'Order History')} (
								{orders.length})
							</button>
							<button
							...
							>
								{t('questionBanks.marketplace.myAccess', 'My Access')} (
								{entitlements.length})
							</button>
						</nav>
					</div>
								...
								{orders.length === 0 ? (
									<div className='text-center py-12'>
										...
										<h3 className='text-lg font-medium text-gray-900 mb-2'>
											{t(
												'questionBanks.marketplace.noOrders',
												'No orders yet'
											)}
										</h3>
										<p className='text-gray-500'>
											{t(
												'questionBanks.marketplace.noOrdersDescription',
												...
												<div className='flex items-start justify-between'>
													<div className='flex-1'>
														...
														<p className='text-sm text-gray-600 mb-2'>
															{order.bankId.description}
														</p>
														<div className='text-xs text-gray-500'>
															<span>
																{order.bankId.questionCount}{' '}
																questions
															</span>
							...
							<div>
								{entitlements.length === 0 ? (
									<div className='text-center py-12'>
										...
										<h3 className='text-lg font-medium text-gray-900 mb-2'>
											{t(
												'questionBanks.marketplace.noAccess',
												'No access granted yet'
											)}
										</h3>
										<p className='text-gray-500'>
											{t(
												'questionBanks.marketplace.noAccessDescription',
												'Question banks you have access to will appear here'
											)}
										</p>
									</div>
								) : (
														...
														<div className='flex items-center space-x-2 mb-2'>
															...
															>
																{entitlement.status
																	.charAt(0)
																	.toUpperCase() +
																	entitlement.status.slice(1)}
															</span>
															<span
																className={getAccessTypeBadge(
															...
															>
																{entitlement.accessType
																	.charAt(0)
																	.toUpperCase() +
																	entitlement.accessType.slice(1)}
															</span>
														</div>
														<p className='text-sm text-gray-600 mb-2'>
															{entitlement.bankId.description}
														</p>
														<div className='text-xs text-gray-500'>
															<span>
																{entitlement.bankId.questionCount}{' '}
																questions
															</span>
															<span className='mx-2'>•</span>
															<span>
													...
													<div className='flex flex-col items-end space-y-2'>
														...
														{entitlement.status === 'active' && (
															<button
																onClick={() =>
																	handleUseBank(
																		entitlement.bankId._id
																	)
																}
																className='px-3 py-1 bg-green-600 text-white text-xs font-medium rounded hover:bg-green-700 transition-colors'
															>
																{t(
																	'questionBanks.marketplace.useNow',
																	'Use now'
																)}
															</button>
														)}
