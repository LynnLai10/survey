...
const mockToken =
	'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpZCI6IjY3NTBkZjI2YzJmZDY4NjI3ZGRlNjQzMCIsImVtYWlsIjoic3VwZXJhZG1pbkBzeXN0ZW0uY29tIiwicm9sZSI6InN1cGVyQWRtaW4iLCJpYXQiOjE3MzMyNzQ5MTgsImV4cCI6MTczMzM2MTMxOH0.test';
...
async function testUserAPI() {
	...
	try {
		...
		try {
			...
			console.log(
				'⚠️ Create user API not available or failed:',
		...
		try {
			...
			console.log(
				`   User: ${userResponse.data.data.name} (${userResponse.data.data.email})`
			);
		...
		try {
			const updateResponse = await axios.put(`${API_BASE}/users/${testUserId}`, updateData, {
				headers,
			});
		...
		try {
			const resetResponse = await axios.post(
				`${API_BASE}/users/${testUserId}/reset-password`,
				{},
				{ headers }
			);
		...
		try {
			...
			console.log(
				`   New status: ${statusResponse.data.data.isActive ? 'Active' : 'Inactive'}`
			);
		...
		try {
			const statsResponse = await axios.get(`${API_BASE}/users/${testUserId}/stats`, {
				headers,
			});
			...
			console.log(
				`   Last Activity: ${new Date(statsResponse.data.data.lastActivity).toLocaleString()}`
			);
		} catch (error) {
