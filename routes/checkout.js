...
router.post('/free', jwtAuth, async (req, res) => {
  try {
    ...
    const resolveUser = async req => {
      ...
      return await User.findOne({ $or: [{ email: raw }, { username: raw }] }).select('_id companyId');
    };
    ...
    const existing = await Order.findOne({ companyId: user.companyId, bankId: bank._id, type: 'free' });
    if (existing) {
