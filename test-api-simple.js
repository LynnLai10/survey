...
async function testAPI() {
	...
	try {
		...
		if (usersResponse.data.data && usersResponse.data.data.length > 0) {
			...
			console.log(
				`   Updated department: ${updateResponse.data.data.department || 'Not set'}`
			);

			// Test password reset
			console.log('\n🔑 Testing password reset...');
			const resetResponse = await axios.post(
				`${API_BASE}/users/${testUser._id}/reset-password`,
				{},
				{ headers }
			);
			...
			const statusResponse = await axios.put(
				`${API_BASE}/users/${testUser._id}/status`,
				{ isActive: newStatus },
				{ headers }
			);
			...
			try {
				...
				if (error.response?.status === 404) {
					...
					console.log(
						`❌ Update endpoint error: ${error.response?.data?.error || error.message}`
					);
				}
			}
		}
		...
		console.log(
			`❌ User management test failed: ${error.response?.data?.error || error.message}`
		);
	}
