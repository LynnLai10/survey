...
async function checkMigrationPrerequisites() {
	...
	const hasCompoundIndex = indexes.some(
		idx => idx.key?.slug === 1 && idx.key?.companyId === 1 && idx.unique
	);
		...
		try {
			...
			if (!error.message.includes('already exists')) {
				throw new MigrationError(
					`Failed to create compound index: ${error.message}`,
					'INDEX_ERROR'
				);
			}
		}
...
async function createDefaultCompany() {
	...
	if (!defaultCompany) {
		if (MIGRATION_CONFIG.DRY_RUN) {
			logger.info(
				`[DRY RUN] Would create default company: ${MIGRATION_CONFIG.DEFAULT_COMPANY_SLUG}`
			);
...
async function handleSlugConflicts(defaultCompanyId) {
	...
	for (const conflict of conflicts) {
		...
		for (let i = 0; i < surveysToRename.length; i++) {
			...
			if (MIGRATION_CONFIG.DRY_RUN) {
				logger.info(
					`[DRY RUN] Would rename survey '${survey.title}' slug: '${slug}' → '${newSlug}'`
				);
				continue;
			}

			await Survey.updateOne({ _id: survey._id }, { $set: { slug: newSlug } });
